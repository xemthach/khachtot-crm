...
?>
        ...
        <div class="form-group">
            ...
            <div class="radio radio-inline radio-primary">
                <input type="radio" name="settings[email_protocol]" id="brevo_api" value="brevo_api"
                    <?= get_option('email_protocol') == 'brevo_api' ? 'checked' : ''; ?>>
                <label for="brevo_api">Brevo API</label>
            </div>
        </div>
        <div
...
echo render_input('settings[smtp_password]', 'settings_email_password', $ps, 'password', ['autocomplete' => 'off']); ?>
        ...
        <div class="brevo-api-fields<?= get_option('email_protocol') == 'brevo_api' ? '' : ' hide'; ?>">
            <?php
            $brevoApiKey = get_option('brevo_api_key');
            if (!empty($brevoApiKey)) {
                $decrypted = $this->encryption->decrypt($brevoApiKey);
                if ($decrypted !== false) {
                    $brevoApiKey = $decrypted;
                }
            }
            echo render_input('settings[brevo_api_key]', 'settings_brevo_api_key', $brevoApiKey, 'password', ['autocomplete' => 'off']);
            ?>
            <p class="text-muted">Brevo API key is encrypted at rest. Leave blank to keep existing key.</p>
        </div>
