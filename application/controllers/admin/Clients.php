    ...
    public function form_contact($customer_id, $contact_id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            try {
                ...
                log_message('error', 'Clients::form_contact failed. customer_id=' . (int) $customer_id . ', contact_id=' . (int) $contact_id . ', error=' . $e->getMessage() . ' at ' . $e->getFile() . ':' . $e->getLine());
                set_status_header(500);
                echo json_encode([
                    'success' => false,
                    'message' => 'Unable to save contact. Check server logs for details.',
                ]);
                die;
            }
        }
