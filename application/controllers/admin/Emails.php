...
class Emails extends AdminController
{
        ...
        foreach ($this->app->get_available_languages() as $avLanguage) {
            if ($avLanguage != 'english') {
                foreach ($email_templates_english as $template) {
                    ...
                    if ($notExists) {
                        $localized = kt_email_template_localized_defaults($template['slug'], $avLanguage, [
                        ...
                        $data['name']      = $localized['name'] ?? ($template['name'] . ' [' . $avLanguage . ']');
                        $data['subject']   = $localized['subject'] ?? $template['subject'];
    ...
    {
        if ($this->input->post()) {
            if (get_option('email_protocol') === 'brevo_api') {
                $to = trim((string) $this->input->post('test_email'));
                $apiKeyEncrypted = (string) get_option('brevo_api_key');
                $apiKey = $this->encryption->decrypt($apiKeyEncrypted);
                if ($apiKey === false) {
                ...
                $result = $this->send_via_brevo_api([
                    ...
                    'to_email' => $to,
                    'from_email' => (string) get_option('smtp_email'),
                    'from_name' => get_option('companyname') != '' ? get_option('companyname') : 'TEST',
                    'subject' => 'Brevo API Setup Testing',
                    'html_content' => get_option('email_header') . 'This is test Brevo API email.' . get_option('email_footer'),
                    'text_content' => 'This is test Brevo API email.',
                ]);
                if (!empty($result['success'])) {
                    set_alert('success', 'Brevo API settings look correct. Check your email now.');
                } else {
                    set_debug_alert('<h1>Brevo API settings are not correct.</h1><br />' . html_escape((string) ($result['message'] ?? 'Unknown error')));
                }
                return;
            }
    ...
    private function send_via_brevo_api(array $payload)
    {
        ...
        curl_close($ch);
        if ($response === false) {
            return ['success' => false, 'message' => $error !== '' ? $error : 'cURL error'];
        }
        $decoded = json_decode((string) $response, true);
        if ($httpCode >= 200 && $httpCode < 300) {
            return ['success' => true, 'message_id' => (string) ($decoded['messageId'] ?? '')];
        }
        return ['success' => false, 'message' => (string) ($decoded['message'] ?? ('HTTP ' . $httpCode))];
    }
