    ...
    public function update_contact($data, $id, $client_request = false)
    {
        ...
        if ($client_request == false) {
            ...
            if ($send_set_password_email) {
                try {
                    ...
                    log_message('error', 'Clients_model::update_contact set_password_email failed for contact_id=' . (int) $id . ', error=' . $e->getMessage());
                    $set_password_email_sent = false;
                }
            }
        }
        ...
        if (($client_request == true) && $send_set_password_email) {
            try {
                ...
                log_message('error', 'Clients_model::update_contact(set client area) set_password_email failed for contact_id=' . (int) $id . ', error=' . $e->getMessage());
                $set_password_email_sent = false;
            }
        }
        ...
        if ($contact_id) {
            ...
            if ($send_welcome_email == true && !empty($data['email'])) {
                try {
                    ...
                    log_message('error', 'Clients_model::add_contact welcome email failed for contact_id=' . (int) $contact_id . ', error=' . $e->getMessage());
                }
            }
            ...
            if ($send_set_password_email) {
                try {
                    ...
                    log_message('error', 'Clients_model::add_contact set_password_email failed for contact_id=' . (int) $contact_id . ', error=' . $e->getMessage());
                }
            }
            ...
            if (defined('CONTACT_REGISTERING')) {
                try {
                    ...
                    log_message('error', 'Clients_model::add_contact verification email failed for contact_id=' . (int) $contact_id . ', error=' . $e->getMessage());
                }
            } else {
                ...
                try {
                    ...
                    log_message('error', 'Clients_model::add_contact transfer_email_tickets_to_contact failed for contact_id=' . (int) $contact_id . ', error=' . $e->getMessage());
                }
            }
