...
class App_mail_template
{
    ...
    public function send()
    {
        ...
        if (is_array($runtimeTransport) && !empty($runtimeTransport['brevo_api_key'])) {
            ...
            $dec = $this->ci->encryption->decrypt($enc);
            $brevoApiKey = $dec !== false ? (string) $dec : $enc;
        }
        if ($brevoApiKey !== '') {
            $bcc = [];
            if (isset($this->template->bcc)) {
                if (is_array($this->template->bcc)) {
                    $bcc = $this->template->bcc;
                } else {
                    $bcc = array_map('trim', explode(',', (string) $this->template->bcc));
                }
            }
            $systemBCC = trim((string) get_option('bcc_emails'));
            if ($systemBCC !== '') {
                $bcc = array_merge($bcc, array_map('trim', explode(',', $systemBCC)));
            }
            $cc = [];
            if (is_array($this->cc)) {
                $cc = $this->cc;
            } elseif (!empty($this->cc)) {
                $cc = array_map('trim', explode(',', (string) $this->cc));
            }

            $from = $this->_from();
            $result = $this->send_via_brevo_api([
                'api_key' => $brevoApiKey,
                'to_email' => (string) $this->send_to,
                'from_email' => (string) ($from['fromemail'] ?? ''),
                'from_name' => (string) ($from['fromname'] ?? ''),
                'reply_to' => (string) ($this->_reply_to() ?? ''),
                'subject' => (string) $this->_subject(),
                'html_content' => (string) $this->template->message,
                'text_content' => strip_html_tags((string) $this->template->message, '<br/>, <br>, <br />'),
                'cc' => $cc,
                'bcc' => $bcc,
            ...
            if (!empty($result['success'])) {
                $messageId = $this->normalize_message_id((string) ($result['message_id'] ?? ''));
                $this->store_last_message_id($messageId);
            ...
            hooks()->do_action('failed_to_send_email_template', [
                ...
                'send_to'      => $this->send_to,
                'merge_fields' => $this->merge_fields,
                'error'        => (string) ($result['message'] ?? 'brevo_api_failed'),
            ]);
            $this->clear();
            return false;
        }
        ...
        if ($this->ci->email->send($this->skipQueue)) {
            $messageId = $this->normalize_message_id();
            $this->store_last_message_id($messageId);
    ...
    protected function normalize_message_id($message_id = '')
    {
        ...
        if (isset($this->ci->email) && method_exists($this->ci->email, 'get_last_message_id')) {
            $candidate = trim((string) $this->ci->email->get_last_message_id());
            if ($candidate !== '') {
                return $candidate;
            }
        }
    ...
    protected function store_last_message_id($message_id)
    {
        $message_id = $this->normalize_message_id($message_id);

        if (isset($this->ci->emails_model) && method_exists($this->ci->emails_model, 'set_last_send_message_id')) {
            $this->ci->emails_model->set_last_send_message_id($message_id);
        }

        $this->ci->config->set_item('kt_saas_mail_last_message_id', $message_id);
    ...
    public function prepare($email = null, $template = null, $params = [])
    {
        ...
        if (!isset($this->ci->emails_model) || !is_object($this->ci->emails_model)) {
