...
class Session_database_driver extends CI_Session_driver implements SessionHandlerInterface
{
    ...
    protected function resolveIpAddress()
    {
        ...
        if (function_exists('get_instance')) {
            ...
            if (isset($CI->input) && method_exists($CI->input, 'ip_address')) {
                $ip = trim((string) $CI->input->ip_address());
                if ($ip !== '') {
                    return $ip;
                }
            }
    ...
    protected function sessionPayloadMatchesCurrentHost($payload)
    {
        if (!is_string($payload) || $payload === '') {
            return true;
        }

        if (!preg_match('/_kt_saas_tenant_host\\|s:\\d+:\"([^\"]*)\";/', $payload, $matches)) {
            return true;
        }

        $storedHost = strtolower(trim((string) ($matches[1] ?? '')));
        ...
        return hash_equals($storedHost, $currentHost);
    }
    ...
    public function read($session_id)
    {
        ...
        if ($this->_config['match_ip']) {
            $this->_db->where('ip_address', $this->resolveIpAddress());
        }
        ...
        if (!$this->sessionPayloadMatchesCurrentHost($result)) {
            $this->_row_exists  = false;
            $this->_fingerprint = md5('');
    ...
    public function validateSessionId($id)
    {
        ...
        empty($this->_config['match_ip']) or $this->_db->where('ip_address', $this->resolveIpAddress());
        $result                   = $this->_db->get();
