...
class App_mailer extends CI_Email
{
    ...
    public function set_last_message_id($message_id)
    {
        $this->last_message_id = trim((string) $message_id);
    ...
    public function get_last_message_id()
    {
        ...
        if ($this->mailer_engine === 'phpmailer' && is_object($this->phpmailer)) {
            if (method_exists($this->phpmailer, 'getLastMessageID')) {
                $messageId = trim((string) $this->phpmailer->getLastMessageID());
                if ($messageId !== '') {
                    return $messageId;
                }
            }

            if (method_exists($this->phpmailer, 'getSMTPInstance')) {
                $smtp = $this->phpmailer->getSMTPInstance();
                if (is_object($smtp) && method_exists($smtp, 'getLastReply')) {
                    $reply = trim((string) $smtp->getLastReply());
                    if ($reply !== '' && preg_match('/queued as ([A-Za-z0-9._:-]+)/i', $reply, $matches)) {
    ...
    protected function normalize_message_id($message_id = '')
    {
        ...
        $candidate = $this->get_last_message_id();
        if ($candidate !== '') {
            return $candidate;
        }
    ...
    public function send($auto_clear = true)
    {
        ...
        if ($this->mailer_engine == 'phpmailer') {
            ...
            if ($result) {
                $this->set_last_message_id($this->normalize_message_id());
