...
/**
 * Determine whether a currency object/string represents VND.
 *
 * @param mixed $currency
 *
 * @return bool
 */
function _currency_is_vnd($currency)
{
    if (is_string($currency)) {
        return strtoupper(trim($currency)) === 'VND';
    }
    ...
    $candidates = [];
    foreach (['name', 'code', 'symbol'] as $field) {
        if (isset($currency->{$field}) && is_string($currency->{$field})) {
            $candidates[] = strtoupper(trim($currency->{$field}));
        }
    }

    return in_array('VND', $candidates, true);
}
