...
function filter_client_visible_tabs($tabs, $id = '')
{
    ...
    $skipBadges = function_exists('kt_saas_is_tenant_runtime') && kt_saas_is_tenant_runtime();
...
function load_client_language($customer_id = '')
{
    ...
    if (defined('CLIENTS_AREA') && get_contact_language() != '' && ! is_language_disabled()) {
        ...
        if (function_exists('kt_saas_is_tenant_runtime') && kt_saas_is_tenant_runtime() && function_exists('kt_saas_resolve_localization_context')) {
            try {
                $runtimeLocalization = kt_saas_resolve_localization_context(['scope' => 'client_language']);
                $resolvedLanguage = trim((string) ($runtimeLocalization['language'] ?? ''));
                if ($resolvedLanguage !== '') {
                    $language = $resolvedLanguage;
                }
            } catch (Throwable $e) {
                log_message('error', 'KT SaaS client language resolver failed: ' . $e->getMessage());
            }
        }
