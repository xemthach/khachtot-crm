...
function payment_gateway_logo_url()
{
    ...
    if (function_exists('kt_saas_resolve_branding_context')) {
        $branding = kt_saas_resolve_branding_context(['scope' => 'payment_gateway']);
        if (is_array($branding)) {
            $logoURL = trim((string) ($branding['dark_logo_url'] ?? ($branding['logo_url'] ?? '')));
        }
    }

    if ($logoURL !== '') {
        return hooks()->apply_filters('payment_gateway_logo_url', $logoURL);
    }

    if (function_exists('kt_saas_is_tenant_runtime') && kt_saas_is_tenant_runtime()) {
        return hooks()->apply_filters('payment_gateway_logo_url', '');
    }
