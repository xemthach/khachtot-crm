...
function load_admin_language($staff_id = '')
{
    ...
    if (function_exists('kt_saas_is_tenant_runtime') && kt_saas_is_tenant_runtime() && function_exists('kt_saas_resolve_localization_context')) {
        try {
            $runtimeLocalization = kt_saas_resolve_localization_context(['scope' => 'admin_language']);
            $resolvedLanguage = trim((string) ($runtimeLocalization['language'] ?? ''));
            if ($resolvedLanguage !== '') {
                $language = $resolvedLanguage;
            }
        } catch (Throwable $e) {
            log_message('error', 'KT SaaS admin language resolver failed: ' . $e->getMessage());
        }
    }
...
function is_admin($staffid = '')
{
    ...
    if ((string) $staffid === (string) $currentStaffId && isset($GLOBALS['current_user'])) {
        return $GLOBALS['current_user']->admin === '1';
    }
    ...
    $CI->db->reset_query();
    $result = $CI->db->select('admin')
        ->from(db_prefix() . 'staff')
        ->where('staffid', $staffid)
        ->limit(1)
        ->get()
        ->row();
    $result = $result && (string) $result->admin === '1';
...
function _maybe_system_setup_warnings()
{
    if (! defined('DISABLE_APP_SYSTEM_HELP_MESSAGES') || (defined('DISABLE_APP_SYSTEM_HELP_MESSAGES') && DISABLE_APP_SYSTEM_HELP_MESSAGES)) {
        hooks()->add_action('ticket_created', function (...$params) {
            (new PopupMessage('app\services\messages\FirstTicketCreated'))->check(...$params);
        });
        hooks()->add_action('lead_created', function (...$params) {
            (new PopupMessage('app\services\messages\FirstLeadCreated'))->check(...$params);
        });
        hooks()->add_action('new_tag_created', function (...$params) {
            (new PopupMessage('app\services\messages\FirstTagCreated'))->check(...$params);
        });
        hooks()->add_action('task_timer_started', function (...$params) {
            (new PopupMessage('app\services\messages\StartTimersWithNoTasks'))->check(...$params);
        });
        hooks()->add_action('task_checklist_item_created', function (...$params) {
            (new PopupMessage('app\services\messages\ReOrderTaskChecklistItems'))->check(...$params);
        });
        hooks()->add_action('smtp_test_email_success', function (...$params) {
            (new PopupMessage('app\services\messages\MailConfigured'))->check(...$params);
        });
    }
    ...
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\ModSecurityEnabled'))->check();
    });
    ...
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\StaticIndexHtml'))->check();
    });
    // Show development message
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\DevelopmentEnvironment'))->check();
    });
    // Check if cron is required to be configured for some features
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\IsCronSetupRequired'))->check();
    });
    // Base url check for https
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\IsBaseUrlChangeRequired'))->check();
    });
    // Check if timezone is set
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\Timezone'))->check();
    });
    // Notice for cloudflare rocket loader
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\CloudFlare'))->check();
    });
    // Notice for iconv extension
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\Iconv'))->check();
    });
    ...
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\TcpdfFileMissing'))->check();
    });
    // Check for cron job running
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\CronJobFailure'))->check();
    });
    // Php version notice
    hooks()->add_action('before_start_render_dashboard_content', function () {
        (new Message('app\services\messages\PhpVersionNotice'))->check();
    });
}
