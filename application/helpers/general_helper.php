...
function maybe_redirect_to_previous_url()
{
    ...
    if ($CI->session->has_userdata('red_url')) {
        ...
        $currentHost = strtolower(trim((string) $currentHost));
        if (strpos($currentHost, ':') !== false) {
            $parts = explode(':', $currentHost);
            $currentHost = $parts[0];
        }

        $redirectHost = parse_url((string) $red_url, PHP_URL_HOST);
        $redirectHost = strtolower(trim((string) $redirectHost));
        if ($redirectHost !== '' && $currentHost !== '' && $redirectHost !== $currentHost) {
            return;
        }
        ...
        Object.defineProperty(window, "$", {
            set: function(value) {
                window.setTimeout(function() {
                    ...
                    if (!jq || typeof jq.each !== 'function') {
                        return;
                    }

                    jq.each(window.deferAfterjQueryLoaded, function(index, fn) {
