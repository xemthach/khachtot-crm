...
function _upload_blocked_extensions()
{
    return ['php', 'phtml', 'php3', 'php4', 'php5', 'php7', 'phar', 'cgi', 'pl', 'py', 'sh', 'svg'];
}

function _upload_mime_map()
{
    return [
        'jpg'  => ['image/jpeg'],
        'jpeg' => ['image/jpeg'],
...
function _upload_detect_mime_type($tmpFilePath)
{
...
function _upload_file_security_allowed($filename, $tmpFilePath, $allowedExtensions = null)
{
    $extension = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
    $blockedExtensions = _upload_blocked_extensions();

    if ($extension === '' || in_array($extension, $blockedExtensions, true)) {
        return false;
    }

    $parts = array_map('strtolower', explode('.', (string) $filename));
    array_pop($parts);
    ...
    if ($allowedExtensions === null) {
        ...
        $allowedExtensions = array_map(static function ($extension) {
    ...
    $mimeMap = _upload_mime_map();
    if (!isset($mimeMap[$extension])) {
        return false;
    }

    $mimeType = _upload_detect_mime_type($tmpFilePath);
    if ($mimeType === '' || !in_array($mimeType, $mimeMap[$extension], true)) {
        return false;
    }
...
function _upload_random_filename($originalName)
{
...
function handle_contract_attachment($id)
{
    ...
    if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
        ...
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            if (!_upload_file_security_allowed($_FILES['file']['name'], $tmpFilePath)) {
                return false;
            }

            _maybe_create_upload_path($path);
            $filename    = unique_filename($path, _upload_random_filename($_FILES['file']['name']));
            $newFilePath = $path . $filename;
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                $CI           = & get_instance();
...
function handle_sales_attachments($rel_id, $rel_type)
{
    ...
    if (isset($_FILES['file']['name'])) {
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            if (!_upload_file_security_allowed($_FILES['file']['name'], $tmpFilePath)) {
                header('HTTP/1.0 400 Bad error');
                echo _l('file_php_extension_blocked');
                return false;
            }
...
function handle_favicon_upload()
{
    ...
    if (isset($_FILES['favicon']['name']) && $_FILES['favicon']['name'] != '') {
        ...
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            ...
            $extension  = $path_parts['extension'];
            $extension  = strtolower($extension);
            if (!_upload_file_security_allowed($_FILES['favicon']['name'], $tmpFilePath, ['ico', 'png'])) {
