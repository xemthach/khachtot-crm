...
function load_pdf_language($clientLanguage)
{
    ...
    if (function_exists('kt_saas_is_tenant_runtime') && kt_saas_is_tenant_runtime() && function_exists('kt_saas_resolve_localization_context')) {
        try {
            $runtimeLocalization = kt_saas_resolve_localization_context(['scope' => 'pdf']);
            $resolvedLanguage = trim((string) ($runtimeLocalization['language'] ?? ''));
            if ($resolvedLanguage !== '') {
                $language = $resolvedLanguage;
            }
        } catch (Throwable $e) {
            log_message('error', 'KT SaaS pdf language resolver failed: ' . $e->getMessage());
        }
    }
...
function pdf_logo_url()
{
    ...
    $logoUrl                   = '';
    $tenantRuntime             = function_exists('kt_saas_is_tenant_runtime') && kt_saas_is_tenant_runtime();
    $branding                  = function_exists('kt_saas_resolve_branding_context')
        ? kt_saas_resolve_branding_context(['scope' => 'pdf'])
        : [];
    ...
    if ($custom_pdf_logo_image_url != '') {
        ...
        $resolvedDarkLogo = trim((string) ($branding['dark_logo'] ?? ''));
        $resolvedLightLogo = trim((string) ($branding['logo'] ?? ''));
        $resolvedDarkLogoPath = trim((string) ($branding['dark_logo_path'] ?? ''));
        $resolvedLightLogoPath = trim((string) ($branding['logo_path'] ?? ''));
        if ($resolvedDarkLogoPath !== '' && file_exists($resolvedDarkLogoPath)) {
            $logoUrl = $resolvedDarkLogoPath;
        } elseif ($resolvedLightLogoPath !== '' && file_exists($resolvedLightLogoPath)) {
            $logoUrl = $resolvedLightLogoPath;
        } elseif (!$tenantRuntime && $resolvedDarkLogo != '' && file_exists($companyUploadPath . $resolvedDarkLogo)) {
            $logoUrl = $companyUploadPath . $resolvedDarkLogo;
        } elseif (!$tenantRuntime && $resolvedLightLogo != '' && file_exists($companyUploadPath . $resolvedLightLogo)) {
            $logoUrl = $companyUploadPath . $resolvedLightLogo;
        } elseif (!$tenantRuntime && get_option('company_logo_dark') != '' && file_exists($companyUploadPath . get_option('company_logo_dark'))) {
            $logoUrl = $companyUploadPath . get_option('company_logo_dark');
        } elseif (!$tenantRuntime && get_option('company_logo') != '' && file_exists($companyUploadPath . get_option('company_logo'))) {
