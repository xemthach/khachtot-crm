...
function get_company_logo($uri = '', $href_class = '', $type = '')
{
    ...
    if (function_exists('kt_saas_resolve_branding_context')) {
        $branding = kt_saas_resolve_branding_context(['scope' => 'ui']);
        if (is_array($branding)) {
            $company_name = trim((string) ($branding['company_name'] ?? ''));
            $company_logo = trim((string) ($type === 'dark' ? ($branding['dark_logo'] ?? '') : ($branding['logo'] ?? '')));
            $company_logo_url = trim((string) ($type === 'dark' ? ($branding['dark_logo_url'] ?? '') : ($branding['logo_url'] ?? '')));
        }
    }

    if ($company_logo === '' && $company_name === '' && !$tenantRuntime) {
    ...
    if ($company_logo != '' && ($company_logo_url !== '' || !$tenantRuntime)) {
        $logo = '<a href="' . $logoURL . '" class="logo img-responsive' . ($href_class != '' ? ' ' . $href_class : '') . '">
        <img src="' . e($company_logo_url !== '' ? $company_logo_url : base_url('uploads/company/' . $company_logo)) . '" class="img-responsive" alt="' . e($company_name) . '">
        </a>';
    } elseif ($company_name != '') {
...
function get_dark_company_logo($uri = '', $href_class = '')
{
    if (function_exists('kt_saas_resolve_branding_context')) {
        $branding = kt_saas_resolve_branding_context(['scope' => 'ui']);
        if (is_array($branding) && trim((string) ($branding['dark_logo'] ?? '')) === '') {
...
function get_admin_header_logo_url()
{
    ...
    if (function_exists('kt_saas_resolve_branding_context')) {
        $branding = kt_saas_resolve_branding_context(['scope' => 'ui']);
        if (is_array($branding)) {
            $logo = trim((string) ($branding['dark_logo'] ?? ($branding['logo'] ?? '')));
            $resolvedLogoUrl = trim((string) ($branding['dark_logo_url'] ?? ($branding['logo_url'] ?? '')));
        }
    }

    if ($logo === '' && !$tenantRuntime) {
    ...
    if ($resolvedLogoUrl !== '') {
        $url = $resolvedLogoUrl;
    } elseif (! empty($logo) && !$tenantRuntime) {
