...
class Einvoice extends AdminController
{
    ...
    public function tenant_settings(): void
    {
        ...
        $canEditSettings = function_exists('kt_saas_feature_allowed')
            ? kt_saas_feature_allowed('einvoice', 'einvoice.settings.edit', true)
            : true;

        if ($this->input->post()) {
            if (!$canEditSettings) {
                set_alert('warning', 'Gói hiện tại không cho phép chỉnh cấu hình e-Invoice của tenant.');
                redirect(admin_url('einvoice/tenant_settings'));
            }

            $result = einvoice_module_save_settings($this->input->post());
            set_alert(!empty($result['success']) ? 'success' : 'warning', (string) ($result['message'] ?? _l('something_went_wrong')));
            redirect(admin_url('einvoice/tenant_settings'));
        }

        $data['title'] = _l('settings_group_einvoice');
        $data['settings'] = einvoice_module_get_settings();
        $data['templates'] = einvioce_module_get_templates();
        $data['can_edit_settings'] = $canEditSettings;
        $this->load->view('einvoice/tenant_settings', $data);
    }
    ...
    public function template($id = ''): void
    {
        ...
        $data['back_url'] = einvoice_module_settings_url();
    ...
    public function validate_and_save($id = null): void
    {
        ...
        if ($success) {
            echo json_encode([
                'success'  => true,
                'message'  => $message,
                'redirect' => einvoice_module_settings_url(),
            ]);
        } else {
            echo json_encode([
                'success' => false,
