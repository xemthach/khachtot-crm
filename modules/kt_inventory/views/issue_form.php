...
<div id="wrapper"><div class="content"><div class="panel_s"><div class="panel-body">
    <a href="<?php echo admin_url('kt_inventory/goods_issues'); ?>" class="btn btn-default"><?php echo _l('kt_inventory_back_to_list'); ?></a>
    ...
    <div class="row">
        <div class="col-md-3"><?php echo render_input('issue_code', 'kt_inventory_code', $header['issue_code'] ?? kt_inventory_generate_code('issue'), 'text', ['readonly' => true]); ?></div>
        ...
        <div class="col-md-3"><?php echo render_input('issue_date', 'kt_inventory_date', $header['issue_date'] ?? date('Y-m-d'), 'date', ['required' => true]); ?></div>
        <div class="col-md-3"><div class="form-group"><label><?php echo _l('kt_inventory_customer'); ?></label><select name="customer_id" class="form-control selectpicker" id="kt_inventory_customer_id"><option value=""></option><?php foreach ($customers as $customer) { ?><option value="<?php echo (int) $customer['userid']; ?>" <?php echo ((int) ($header['customer_id'] ?? 0) === (int) $customer['userid']) ? 'selected' : ''; ?>><?php echo html_escape($customer['company']); ?></option><?php } ?></select></div></div>
    </div>
    <div class="row">
        <div class="col-md-4"><div class="form-group"><label><?php echo _l('kt_inventory_invoice'); ?></label><select name="invoice_id" class="form-control selectpicker" id="kt_inventory_invoice_id"><option value=""></option><?php foreach ($invoices as $invoice) { ?><option value="<?php echo (int) $invoice['id']; ?>" data-customer-id="<?php echo (int) ($invoice['clientid'] ?? 0); ?>" <?php echo ((int) ($header['invoice_id'] ?? 0) === (int) $invoice['id']) ? 'selected' : ''; ?>>#<?php echo (int) ($invoice['number'] ?: $invoice['id']); ?><?php echo !empty($invoice['company']) ? ' - ' . html_escape($invoice['company']) : ''; ?></option><?php } ?></select></div></div>
        <div class="col-md-8"><?php echo render_textarea('note', 'kt_inventory_note', $header['note'] ?? ''); ?></div>
    </div>
    <div class="row">
    ...
    <script type="text/template" id="issue-lines-template">
    ...
    <button type="button" class="btn btn-default" data-kt-add-line="issue-lines"><?php echo _l('kt_inventory_add_new'); ?></button>
