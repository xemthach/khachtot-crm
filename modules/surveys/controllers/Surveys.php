...
class Surveys extends AdminController
{
    ...
    public function import_emails()
    {
        ...
        $uploadValidation = $this->validateSurveyImportUpload($_FILES['file_xls'] ?? []);
        if (!$uploadValidation['success']) {
            set_alert('danger', $uploadValidation['message']);
            redirect(admin_url('surveys/mail_list_view/' . $listid));
        }

        $filename = uniqid() . '_' . preg_replace('/[^A-Za-z0-9._-]/', '_', $_FILES['file_xls']['name']);
    ...
    private function validateSurveyImportUpload(array $file)
    {
        if (empty($file['name']) || empty($file['tmp_name']) || !empty($file['error'])) {
            return ['success' => false, 'message' => _l('error_uploading_file')];
        }
        ...
        if ($extension === '' || in_array($extension, $blockedExtensions, true) || !isset($allowedMimes[$extension])) {
            return ['success' => false, 'message' => _l('file_php_extension_blocked')];
        }

        $parts = array_map('strtolower', explode('.', (string) $file['name']));
        array_pop($parts);
        ...
        $mimeType = $this->detectSurveyImportMimeType((string) $file['tmp_name']);
    ...
    private function detectSurveyImportMimeType($path)
    {
